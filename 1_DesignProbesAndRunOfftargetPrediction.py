...
#Create a folder named 'part1' within the current working directory, this will be where all probes are saved and the sub_sam files.
...
#Loading in the gene names from the reference database. This will throw error messages if the gene name is not present (e.g., eGFP)
#but is also a good measure in case the gene name is not exactly the same as the reference database (e.g., Cd8 vs. CD8). 
...
for target in targets:
	...
	#Nothing happens if target not in reference, just throws a warning. Good for trouble shooting if gene names are not an exact match (e.g., Cd8 vs. CD8)
	if target_name not in database_gene_list:
		print('Is this the correct reference database? Your sequence {} is not found in the reference.'.format(target_name))
	
	seq = target.seq
	sub_seq_list = []
	
	#iterate over all 25 bp windows, avoid homotetramers, ensure proper junction use, and avoid excessively low or high GC use
	...
	temp_probe_list = open('./part1/{}_AllProbes.fasta'.format(target_name), 'w')
	pre_triage_probe_dict = {}
	for hit in sub_seq_list:
		temp_probe_list.write('>{}_{}\n{}\n'.format(hit['Name'], int(hit['Tm']), hit['Sequence']))
		pre_triage_probe_dict['{}_{}'.format(hit['Name'], int(hit['Tm']))] = hit['Sequence']
	...
	try:
		...
		pass
	pruned_bowtie_results = open('./part1/{}.sub_sam'.format(target_name), 'w')
	...
	for result in bowtie_output:	
		if result[0] != '@':
			...
			if mismatches <= 6:
				try:
					pruned_bowtie_results.write('{}\t{}\t{}\t{}\n'.format(probe_name, hit_gene_name, hit_name, mismatches))
				except:
					print('error')
